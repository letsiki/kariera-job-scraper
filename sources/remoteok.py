...
API: GET https://remoteok.com/api -> JSON array. The first element is
feed metadata (a `{ "legal": ... }` blob); skip it.
"""
...
def _parse_when(raw: dict) -> datetime:
    """RemoteOK provides both `epoch` and `date`. Prefer `epoch`."""
    epoch = raw.get("epoch")
...
def fetch() -> list[JobAd]:
    ...
    for raw in items:
        try:
            ...
            logger.exception("remoteok: failed to normalize an item")
            continue
        if ad is not None:
            out.append(ad)
    return out
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    ads = fetch()
    print(f"remoteok: {len(ads)} candidates after filtering")
    if args.dry_run:
        for ad in ads[:5]:
